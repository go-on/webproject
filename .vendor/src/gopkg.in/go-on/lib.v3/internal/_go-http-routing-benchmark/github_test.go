...
func BenchmarkGoon_GithubStatic(b *testing.B) {
	req, _ := http.NewRequest("GET", "/user/repos", nil)
	benchRequest(b, githubGoon, req)
}
...
func BenchmarkGoon_GithubParam(b *testing.B) {
	req, _ := http.NewRequest("GET", "/repos/julienschmidt/httprouter/stargazers", nil)
	benchRequest(b, githubGoon, req)
}
...
func BenchmarkGoon_GithubAll(b *testing.B) {
	benchRoutes(b, githubGoon, githubAPI)
}
